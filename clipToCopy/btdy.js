...
function mainJob() {
	var ul = this.getElementByAttr('div', 'class', 'list_su').childNodes[1];
	var ulChilds = ul.childNodes;
	console.log(ulChilds)
    for (var i = 0; i < ulChilds.length; i++) {
		var score = parseFloat(ulChilds[i].childNodes[1].childNodes[0].childNodes[1].innerHTML);
		if (score < 7)
		{
...
Insert.prototype.mainJob = mainJob;
