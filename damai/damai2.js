...
function damaiOrder() {
	var proDiv = this.getElementByAttr('div', 'class', 'perform__bd');
	//console.log(proDiv);
	if (this.isStart || proDiv == null){
		var order = this.getElementByAttr('div', 'data-spm-click','gostr=/damai_m.default.click;locaid=dbuynow');
		if (order != null && (!this.isOrder)){
			console.log(order);
		...
		var order2 = this.getElementByAttrAll('div', 'class', 'dm-button dm-button__primary dm-button__large');
		if (order2.length == 2) { 
			console.log(order2);
			order2[1].click();
		}
		
		var ticket = this.getElementByAttrAll('li', 'class', 'price-list__item');
		if (ticket.length != 0 && (!this.isTicket)) {
			/*
			var min = window.localStorage.getItem('min');
			var max = window.localStorage.getItem('max');
			for (var i = 0; i < ticket.length; i++){
				var price = parseInt(ticket[i].innerHTML.match(/\d+/)[0]);
			...
			ticket[ticket.length - 1].click();
			this.isTicket = true;
		}
		var ok = this.getElementByAttr('h2', 'class', 'settlement__button__text');
		if (ok != null) {
			console.log(ok);
			ok.click();
		}
		
		var goBuy = this.getElementByAttr('div', 'class', 'dm-button btn-group__btn dm-button__primary dm-button__large');
		if (goBuy != null) {
			console.log(goBuy);
			goBuy.click();
		}
		...
		if (!this.isAddChoice){
			...
			var rechild = this.getElementByAttr('div', 'class', 'ticketinfo');
			proDiv.insertBefore(this.getChoice(), rechild);
		}
	}
}
...
function setTimer(){
    this.timer = setInterval(this.timerFunc(), 100 * 1);
}
