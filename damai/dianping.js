...
function findChildByAttr(father, attr, value) {
...
function OrderTicket() {
    var order = this.getElementByAttr('div', 'class', 'buy can_buy seatbtn3');
    if (order != null) {
    ...
    var price = this.getElementByAttrAll('div', 'class', 'price-item');
    //price[price.length - 1].click();
    console.log(price);
    
    var confirm = this.getElementByAttr('div', 'class', 'confirm-ticket');
    if (confirm != null) {
        confirm.click();
    }
    
    var willBy = this.getElementByAttr('button', 'class', 'primary-btn-md next');
    if (willBy != null) {
        willBy.click();
    }
    
    //var zhifubao = this.getElementByAttrAll('input', 'name', 'pay-method');
...
Insert.prototype.OrderTicket = OrderTicket;
